...
object STM {
  ...
  final class AtomicallyPartiallyApplied[F[_]] {
    ...
    def apply[A](stm: STM[A])(implicit F: Concurrent[F]): F[A] = {
      var pending: Iterator[Txn] = null
      for {
        run <- F.async { (cb: (Either[Throwable, A] => Unit)) =>
          def attempt: Pending =
            () => {
        ...
        _ <- if (pending != null) rerunPending(pending) else F.unit
      } yield run
    }
    ...
    private def collectPending(log: TLog): Iterator[Txn] = {
    ...
    private def rerunPending(pending: Iterator[Txn])(implicit F: Concurrent[F]): F[Unit] = {
      var acc = F.unit
      while (pending.hasNext) {
        acc = acc >> F.delay(pending.next.pending())
      }
      acc
    }
  }
